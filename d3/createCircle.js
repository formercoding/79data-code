...
Circle.prototype = {
	...
	bindScale: function() {
		...
		document.body.addEventListener("mousewheel", function(event){
			if (event.target != self.svg[0][0] && event.target.parentNode.parentNode != self.svg[0][0]) {
				return false;
			}
...
var myData = 
 	...
 	console.log(d3.selectAll(".year-demo")[0]);
 	var wrap = d3.selectAll(".year-demo")[0][0];
 	var wrap1 = d3.selectAll(".year-demo")[0][1];
 	//console.log(d3.selectAll(".year-demo"));
 	var monthDiv = d3.select(".month-event")[0][0];
 	var eventDiv = d3.select(".event-event")[0][0];
 	var circle = new Circle(wrap, myData, monthDiv);
 	var circle1 = new Circle(wrap1, myData, monthDiv);
